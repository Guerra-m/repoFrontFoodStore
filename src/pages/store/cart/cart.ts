...
function attachEvents() {
    ...
    btn.addEventListener("click", async (e) => {
      ...
      if (action === "increase") {
        ...
        if (item.qty > 1) {
          ...
          if (confirmed) {
            removeFromCart(productId); // eliminamos del carrito
            renderCart();
          }
        }
      }
  ...
  document.querySelectorAll(".btn-remove").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      const target = e.target as HTMLButtonElement;
      const productId = Number(target.dataset.id);
      const nombre = target.dataset.nombre || "este producto";
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      try {
        for (const item of cart) {
          ...
          if (productInDb.stock < item.qty) {
            alert(
              `Stock insuficiente para ${item.nombre}. Disponible: ${productInDb.stock}, en carrito: ${item.qty}`
            );
            return;
          }
        }
      ...
      const confirmed = await customConfirmGeneral(
        `¿Confirmar pedido por $${total.toFixed(2)}?`,
        "Confirmar Pedido"
      );
      if (confirmed) {
