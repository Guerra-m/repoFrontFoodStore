...
function row(p: IProduct): string {
  return `<tr data-id='${p.id}'>
    ...
    <td>${
      p.imagen
        ? `<img src='${p.imagen}' alt='${p.nombre}' style='width:50px;height:50px;object-fit:cover;border-radius:8px'>`
        : ""
    }</td>
    <td>${p.nombre}</td>
    <td>${p.descripcion || ""}</td>
    ...
    <td>${
      p.disponible
        ? '<span style="color:#00b894">✓ Disponible</span>'
        : '<span style="color:#d63031">✗ No disponible</span>'
    }</td>
    <td>
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    if (editandoId === null) {
      ...
      customAlertGeneral("Producto creado ", "¡Éxito!", "success");
    } else {
      await PUT(`/productos/${editandoId}`, payload);
      //alert('Producto actualizado exitosamente');
      customAlertGeneral("Producto actualizado", "¡Éxito!", "success");
    }
    ...
    btn.addEventListener("click", async (e) => {
      ...
      if (confirmed) {
        try {
          ...
          customAlertGeneral("Producto eliminado", "¡Éxito!", "success");
